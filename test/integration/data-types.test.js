...
describe(Support.getTestDialectTeaser('DataTypes'), () => {
  ...
  const testSuccess = async function (Type, value, options) {
    ...
    if (options && options.useBindParam) {
      bindParam = Type.constructor.prototype.bindParam = sinon.spy(function () {
        return Reflect.apply(Sequelize.ABSTRACT.prototype.bindParam, this, arguments);
      });
    }
  ...
  it('calls parse and stringify for UUID', async () => {
    ...
    if (['postgres', 'sqlite', 'db2', 'ibmi', 'yugabyte'].includes(dialect)) {
  ...
  it('calls parse and stringify for CIDR', async () => {
    ...
    if (['postgres', 'yugabyte'].includes(dialect)) {
  ...
  it('calls parse and stringify for CITEXT', async () => {
    ...
    // console.log(Type);
    if (dialect === 'sqlite' || dialect === 'yugabyte') {
      // The "field type" sqlite returns is TEXT so is covered under TEXT test above
      // yugabyte doesn't support CITEXT yet so it will have the type as TEXT
      return;
    }
  ...
  it('calls parse and stringify for ENUM', async () => {
    ...
    if (['postgres', 'db2', 'yugabyte'].includes(dialect)) {
  ...
  if (current.dialect.supports.GEOMETRY) {
    ...
    it('should parse an empty GEOMETRY field', async () => {
      ...
      if (current.dialect.supports.GEOMETRY && runTests) {
        ...
        } else if (['postgres', 'postgres-native', 'yugabyte'].includes(dialect)) {
  ...
  if (['postgres', 'mysql', 'yugabyte'].includes(dialect)) {
  ...
  if (dialect === 'postgres') { // INDEX on INT4RANGE not yet supported in Yugabyte
  ...
  if (current.dialect.supports.RANGE && dialect !== 'yugabyte') { // Index on user_defined_column_type not yet supported in Yugabyte
